...
public class EmulationSurfaceView extends SurfaceView {
    ...
    private static final int[] axisCodes = new int[] {
        ...
        MotionEvent.AXIS_RX,            // 2/RightX
        MotionEvent.AXIS_RY,            // 3/RightY
        MotionEvent.AXIS_LTRIGGER,      // 4/L2
        MotionEvent.AXIS_RTRIGGER,      // 5/R2
        MotionEvent.AXIS_Z,             // 6
        MotionEvent.AXIS_RZ             // 7
    };
    ...
    public static int getAxisIndexForAxisCode(int axisCode) {
        for (int axisIndex = 0; axisIndex < buttonKeyCodes.length; axisIndex++) {
            if (buttonKeyCodes[axisIndex] == axisCode)
                return axisIndex;
        }
    ...
    private boolean addControllerAxisMapping(int deviceId, List<InputDevice.MotionRange> motionRanges, int axis, int controllerIndex) {
        ...
        // special case: map hats to dpad
        if (axis == MotionEvent.AXIS_HAT_X) {
            negativeButton = 13;    // Left
            positiveButton = 14;    // Right
        } else if (axis == MotionEvent.AXIS_HAT_Y) {
            negativeButton = 11;    // Up
            positiveButton = 12;    // Down
        }
    ...
    public boolean initControllerMapping(String controllerType) {
        ...
        for (int deviceId : deviceIds) {
            ...
            if (motionRanges != null) {
                ...
                // special cases
                addControllerAxisMapping(deviceId, motionRanges, MotionEvent.AXIS_HAT_X, controllerIndex);
                addControllerAxisMapping(deviceId, motionRanges, MotionEvent.AXIS_HAT_Y, controllerIndex);
            }
        }
